...
namespace SECYAN {
    ...
    void Relation::Sort() {
        ...
        if (!IsDummy()) {
            ...
            std::sort(indices.begin(), indices.end(),
                      [&](uint32_t i, uint32_t j) { return m_Tuples[i] < m_Tuples[j]; });
    ...
    void Relation::ProjectNames(std::vector<std::string> &projectAttrNames) {
        Project(projectAttrNames);
    }
...
//    }
    ...
    void Relation::SemiJoin(Relation &child, std::vector<std::string> &parentAttrNames,
    ...
    void Relation::SemiJoinAttr(Relation &child, const char *parentAttrName, const char *childAttrName) {
        SemiJoin(child, parentAttrName, childAttrName);
    }

    void Relation::SemiJoinAttrs(Relation &child, std::vector<std::string> &parentAttrNames,
                                 std::vector<std::string> &childAttrNames) {
        SemiJoinAttrs(child, parentAttrNames, childAttrNames);
    }
    ...
    void Relation::AnnotMul(uint32_t *indicator, uint32_t *childAnnotPermuted, bool isChildAnnotBool) {
        ...
        auto s_payload2 = isChildAnnotBool ? bc->PutSharedSIMDINGate(size, childAnnotPermuted, 1)
                                           : ac->PutSharedSIMDINGate(size, childAnnotPermuted, 32);
        share *s_payload1;
        if (m_AI.isBoolean) {
            if (m_AI.knownByOwner)
    ...
    void Relation::OblivSemiJoin(Relation &child, std::vector<std::string> &parentAttrNames,
                                 std::vector<std::string> &childAttrNames) {
        ...
        if (m_RI.isPublic) {
            ...
            if (m_AI.isBoolean)
                ...
                if (gParty.GetRole() == CLIENT)
                    for (uint32_t i = 0; i < numRows; i++)
                        annot[i] = -m_Annot[i];
                zeroAnnot = bc->PutEQGate(bc->PutSIMDINGate(numRows, m_Annot.data(), 32, SERVER),
                                          bc->PutSIMDINGate(numRows, annot.data(), 32, CLIENT));
            }
     ...
     * @param childAttrName
     */
    void Relation::JoinAttr(Relation &child, const char *parentAttrName, const char *childAttrName) {
        Join(child, parentAttrName, childAttrName);
    }
     ...
     * For python wrapper
     * @param child
     * @param parentAttrNames
     * @param childAttrNames
     */
    void Relation::JoinAttrs(Relation &child, std::vector<std::string> &parentAttrNames,
                             std::vector<std::string> &childAttrNames) {
    ...
    void Relation::Join(Relation &child, std::vector<std::string> &parentAttrNames,
                        std::vector<std::string> &childAttrNames) {
        assert(parentAttrNames.size() == childAttrNames.size());
    ...
    void Relation::Union(Relation &child) {
        assert(m_RI.owner == child.m_RI.owner && m_RI.attrNames == child.m_RI.attrNames &&
               m_RI.attrTypes == child.m_RI.attrTypes);
